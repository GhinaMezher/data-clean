...
life_birth|>mutate(diff=abs(FactValueNumeric-Value), keep="name")|>
  summarise(max(diff))
```
check Dim1ValueCode on prof's code.
```{r}
life_birth|>
...
life_reformating<-function(life_df){
  ...
  parent_codes<-life_df|>distinct(ParentLocationCode, ParentLocation)|>
    remane(Region=ParentLocationCode, RegionName=ParentLocation)
  ...
  life<-life_df|>
    ...
    rename(Region= ParentLocationCode, Country=SpatialDimValue)|>
    select(where(\(.x)all(is.na(.x))))
  single_values<-life|>
    select(where(single_value))|> slice_head()
  life<-life|>
    select(!where(single_value))|>
    seelct(-FactValNumeric,-DimValueCode,-DataModified)|>
    rename(Year=Period,
           Attribute=Dim1)
